...
checks = {
    ...
    'changed-input replay fails closed': SQL.count('ACP replay input digest mismatch') == 2,
    'cross-operation action reuse fails closed': SQL.count('ACP action_id already consumed by different operation') == 2,
    'actor evidence exact repository enforced in DB': "p_evidence->>'repository' <> 'lundus88/fieldgis-reference'" in SQL,
    'actor evidence exact workflow enforced in DB': 'vl-agent-control-plane-admin.yml@refs/heads/main' in SQL,
    'actor evidence exact main ref enforced in DB': "p_evidence->>'ref' <> 'refs/heads/main'" in SQL,
