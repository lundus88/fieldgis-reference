...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const {data,error}=await service.rpc("vl_submit_customer_app_request",{
      p_project_id:projectId,
      p_user_id:user.id,
      p_prompt:compilerPrompt,
      p_title:title,
      p_launch:launch
    });
    if(error) return J({error:error.message,blocked:true,production_locked:true},409);

    return new Response(JSON.stringify({ok:true,result:data,production_locked:true,human_production_approval_required:true}),{status:200,headers:{"content-type":"application/json","cache-control":"no-store","access-control-allow-origin":"*"}});
  }catch(e){
    return J({error:String((e as Error).message||e),blocked:true,production_locked:true},400);
  }
});
