...
VALIDATION = json.loads(
    (HERE / "independent-validation.json").read_text(encoding="utf-8")
)

module_spec = importlib.util.spec_from_file_location(
    "evidence_capture",
    MATURITY / "evidence_capture.py",
)
...
def main() -> int:
    ...
    record = capture.finalize_capture(
        ...
        qa_security_evidence=[
            ...
            {
                "kind": "bounded-multi-agent",
                "status": "PASS",
                "source_sha": EXEC["github_sha"],
                "worker_count": EXEC["multi_agent_delegation"]["worker_count"],
            },
        ],
        ...
        multi_agent_delegation=EXEC["multi_agent_delegation"],
    )
    ...
    if record.get("promotion_allowed") is not False:
        raise SystemExit("FAIL: promotion must remain blocked")
    if (
        record.get("multi_agent_delegation", {}).get("demonstrated") is not True
        or record.get("multi_agent_delegation", {}).get("authority_expanded")
        is not False
    ):
        raise SystemExit("FAIL: bounded delegation evidence missing")
    return 0
