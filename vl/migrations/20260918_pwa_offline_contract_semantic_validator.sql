...
begin
  ...
  if exists(select 1 from jsonb_array_elements(v_required) g where g->>'key'='database_security') then
    ...
    set status=case when v_rls_off=0 and v_bad_definer=0 then 'pass' else 'fail' end,
        score=case when v_rls_off=0 and v_bad_definer=0 then 1 else 0 end,
        evidence=jsonb_build_object(
          'scope','vrs_control_plane_database',
          'public_tables_without_rls',v_rls_off,
          'public_security_definer_exposed_to_client_roles',v_bad_definer,
    ...
    v_results:=v_results||jsonb_build_array(
      jsonb_build_object(
        ...
        'status',case when v_rls_off=0 and v_bad_definer=0 then 'pass' else 'fail' end
      )
    );
  end if;
  ...
  if exists(select 1 from jsonb_array_elements(v_required) g where g->>'key'='rollback_contract') then
    select exists(
      select 1
      from public.factory_artifacts fa
      where fa.factory_run_id=v_run.id
        and lower(fa.sha256)=lower(v_dep.artifact_sha256)
    ) into v_artifact_match;

    update public.release_gates
    set status=case when v_run.production_locked=true and v_dep.status in ('planned','certified') and v_artifact_match then 'pass' else 'fail' end,
        ...
        evidence=jsonb_build_object(
          'scope','control_plane_rollback_contract',
          'immutable_artifact_hash_match',v_artifact_match,
          'production_lock_preserved',v_run.production_locked,
          'deployment_status',v_dep.status,
          'rollback_status_supported',true,
          'required_gates_authority','deployment_snapshot',
          'deployment_id',v_dep.id,
          'note','External target rollback remains adapter-specific and is required before deployed state.'
        ),
        checked_at=now(),
        checked_by=null
    ...
    v_results:=v_results||jsonb_build_array(
      jsonb_build_object(
        'key','rollback_contract',
        'status',case when v_run.production_locked=true and v_dep.status in ('planned','certified') and v_artifact_match then 'pass' else 'fail' end
      )
    );
  end if;

  if exists(select 1 from jsonb_array_elements(v_required) g where g->>'key'='offline_pwa_contract') then
    select content into v_sw
    from public.generated_artifacts
    where factory_run_id=v_run.id and path='web/public/sw.js'
    order by created_at desc
    limit 1;

    select content into v_manifest
    from public.generated_artifacts
    where factory_run_id=v_run.id and path='web/public/manifest.webmanifest'
    order by created_at desc
    limit 1;
    ...
    update public.release_gates
    set status=case when v_pwa_offline_ok then 'pass' else 'fail' end,
    ...
    v_results:=v_results||jsonb_build_array(
      jsonb_build_object(
        'key','offline_pwa_contract',
        'status',case when v_pwa_offline_ok then 'pass' else 'fail' end
      )
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'deployment_id',v_dep.id,
    'required_gates_authority','deployment_snapshot',
    'required_gates_snapshot',v_required,
    'server_gate_results',v_results
  );
end
$function$;
