-- Fix false GPS capability inference for manual coordinate input.
-- `coordinate`, `coordinates`, and `koordinat` alone are data-input concepts,
-- not evidence that the app requires device geolocation/GPS hardware.
...
begin
  ...
  -- Device-location capability must be explicit. Manual coordinate entry/conversion
  -- must not force GPS routing.
  if v_prompt ~ '(\mgps\M|\mgnss\M|current location|device location|live location|track(ing)? (my )?position|track(ing)? (my )?location|phone location|telefon.*lokasi|lokasi semasa|kedudukan semasa)' then
    v_caps := array_append(v_caps,'gps');
  end if;
  ...
  if v_prompt ~ '(supabase|database|database sync|sync)'
     and v_prompt !~ '(no database|without database|no supabase|without supabase|database disabled|supabase disabled|no database entities|without database entities)' then
    v_caps := array_append(v_caps,'supabase');
  end if;
  ...
  v_spec := jsonb_build_object(
    ...
    'inferred',jsonb_build_object('builder_type',v_builder_type,'target',v_target,'required_capabilities',v_caps,'auth_requested',v_auth_requested),
    'routing',v_route,
