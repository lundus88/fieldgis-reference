...
def evidence(**kw):
    d=dict(
        ...
        amount=100.0,currency="MYR",payment_reconciled=True,tax_einvoice_required=True
    )
    d.update(kw)
...
class AccountingAdapterTests(unittest.TestCase):
    ...
    def test_payment_requires_authoritative_reconciliation(self):
        p=self.full_provider()
        self.assertEqual(p.record_payment("p",evidence(payment_reconciled=False))["reason"],"AUTHORITATIVE_PAYMENT_RECONCILIATION_REQUIRED")

    def test_receipt_requires_paid_reconciliation(self):
        p=self.full_provider()
        self.assertEqual(p.create_receipt_reference("r",evidence(payment_reconciled=False))["reason"],"PAID_RECONCILIATION_REQUIRED")

    def test_tax_einvoice_requires_validation(self):
        p=self.full_provider()
        e=evidence()
        inv=p.create_invoice("i",e)
        self.assertEqual(p.reconcile(e,inv["provider_invoice_ref"],"SUBMITTED")["reason"],"EINVOICE_VALIDATION_REQUIRED")

    def test_non_tax_flow_can_reconcile(self):
        p=self.full_provider()
        e=evidence(tax_einvoice_required=False)
        inv=p.create_invoice("i",e)
        self.assertEqual(p.reconcile(e,inv["provider_invoice_ref"],"NOT_REQUIRED")["status"],"PASS")
